...
class browser extends uploader {
    ...
    protected function act_expand() {
        return json_encode(array('dirs' => $this->getDirs($this->toGBK($this->postDir()))));
    }

    protected function act_chDir() {
        $this->postDir(); // Just for existing check
        $this->session['dir'] = "{$this->type}/{$this->toGBK($_POST['dir'])}";
        $dirWritable = dir::isWritable("{$this->config['uploadDir']}/{$this->session['dir']}");
    ...
    protected function act_newDir() {
        ...
        $dir = $this->postDir();
        $newDir = $this->fromGBK($this->normalizeDirname(trim($_POST['newDir'])));
        if (!strlen($newDir))
		...
		$gbkDir = $this->toGBK($dir . '/' . $newDir);
        if (file_exists("$gbkDir"))
            $this->errorMsg("A file or folder with that name already exists.");
        if (!@mkdir("$gbkDir", $this->config['dirPerms']))
    ...
    protected function act_renameDir() {
        ...
        $dir = $this->postDir();
        $newName = $this->fromGBK($this->normalizeDirname(trim($_POST['newName'])));
        if (!strlen($newName))
            $this->errorMsg("Please enter new folder name.");
        ...
        if (substr($newName, 0, 1) == ".")
            $this->errorMsg("Folder name shouldn't begins with '.'");
        if (!@rename($this->toGBK($dir), $this->toGBK(dirname($dir) . "/$newName")))
            $this->errorMsg("Cannot rename the folder.");
        $thumbDir = "$this->thumbsTypeDir/{$_POST['dir']}";
        if (is_dir($this->toGBK($thumbDir)))
            @rename($this->toGBK($thumbDir), $this->toGBK(dirname($thumbDir) . "/$newName"));
    ...
    protected function act_deleteDir() {
        ...
        $thumbDir = "$this->thumbsTypeDir/{$this->toGBK($_POST['dir'])}";
    ...
    protected function act_upload() {
        ...
        if (!dir::isWritable($this->toGBK($dir)))
    ...
    protected function act_download() {
        ...
        if (!isset($_POST['dir']) ||
            ...
            (false === ($file = $this->toGBK("$dir/{$_POST['file']}"))) ||
        ...
        header('Content-Disposition: attachment; filename="' . urlencode(str_replace('"', "_", $_POST['file'])) . '"');
    ...
    protected function act_rename() {
        ...
        if (!$this->config['access']['files']['rename'] ||
            ...
            !$this->checkFilename($_POST['file']) ||
            !$this->checkFilename($_POST['newName']) ||
            (false === ($file = "$dir/{$this->toGBK($_POST['file'])}")) ||
    ...
    protected function act_downloadDir() {
        ...
        new zipFolder($file, $this->toGBK($dir));
    ...
    protected function moveUploadFile($file, $dir) {
        ...
        $target = "$dir/" . file::getInexistantFilename($this->fromGBK($filename), $dir);

        if (!@move_uploaded_file($file['tmp_name'], $this->toGBK($target)) &&
            !@rename($file['tmp_name'], $this->toGBK($target)) &&
            !@copy($file['tmp_name'], $this->toGBK($target))
        ) {
            @unlink($file['tmp_name']);
            return $this->htmlData($file['name']) . ": " . $this->label("Cannot move uploaded file to target folder.");
        } elseif (function_exists('chmod'))
            chmod($this->toGBK($target), $this->config['filePerms']);

        $this->makeThumb($this->toGBK($target));
        ...
        foreach ($files as $file) {
            ...
            if ($type !== false) {
                ...
                if (is_array($size) && count($size)) {
                    $thumb_file = "$thumbDir/" . $this->fromGBK(basename($file));
            ...
            $name = $this->fromGBK(basename($file));
            $ext = file::getExtension($file);
    ...
    protected function postDir($existent=true) {
        ...
        if (!$this->checkFilePath($dir))
            $this->errorMsg("Unknown error.");
        if ($existent && (!is_dir($this->toGBK($dir)) || !is_readable($this->toGBK($dir))))
    ...
    protected function getDirInfo($dir, $removable=false) {
        ...
        $writable = dir::isWritable($dir);
        $info = array(
            'name' => stripslashes($this->fromGBK(basename($dir))),
