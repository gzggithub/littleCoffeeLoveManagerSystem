...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct() {
		...
		//Shalves at 2015-10-17
		//配置文件默认语言支持
		if (!$lngUrlSet && isset($_CONFIG['lng']))
			$this->lang = $_CONFIG['lng'];
	...
	public function toGBK($input) {
		return mb_convert_encoding($input, "GBK");
	}
	
	public function fromGBK($input) {
        ...
        if ($config['disabled'] || !$config['access']['files']['upload']) {
            ...
            if (!strlen($message)) {
                ...
                if (!@move_uploaded_file($file['tmp_name'], $target) &&
                    ...
                    $message = $this->label("Cannot move 123 uploaded file to target folder.");
                else {
    ...
    protected function normalizeDirname($dirname) {
        ...
        return $this->toGBK($dirname);
    }

    protected function checkFilePath($file) {
        $rPath = realpath($this->toGBK($file));
