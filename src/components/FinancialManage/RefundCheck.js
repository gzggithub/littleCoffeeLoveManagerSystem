...
const ItemCheck01Form = Form.create()(
    (props) => {
        const {visible, onCancel, onCreate, form, opinionStatus, setOpinionStatus, institutionAmount, signIn, type, confirmLoading} = props;
        const {getFieldDecorator} = form;
        // 订单类型不为2且signIn为true时，显示退款金额项
        let amountStrClass = "amountStr itemNone";
        if (type !== 2 && signIn) {
            amountStrClass = "amountStr"
        }
        // 审核意见项类名，初始设为非必填
            ...
            >
                <div className="refundCheck-form item-form">
                    <Form layout="vertical">
                        ...
                        {/*signIn为true时，具体退款金额由机构裁定，且不可超过institutionAmount*/}
                        <FormItem className={amountStrClass} {...formItemLayout_8} label="退款金额：">
                            {getFieldDecorator('amountStr', {
                                rules: [{
                                    required: type === 2 ? false : signIn,
                                    message: '审核意见不能为空',
                                }],
                            })(
                                <InputNumber min={0} max={Number(institutionAmount)} precision={2} step={1}/>
                            )}
                        </FormItem>
                        <p className="hint"
                           style={{display: type !== 2 && signIn ? "block" : "none"}}>最大退款金额：{institutionAmount}</p>
        ...
        return (
            <a style={{display: this.props.status ? "inline" : "none"}}>
                ...
                <ItemCheck01Form
                    ...
                    institutionAmount={this.props.institutionAmount}
...
class DataTable extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pagination: {
                current: 1,
                pageSize: Number(localStorage.refundCheckPageSize) || 10,
                pageSizeOptions: ["5", "10", "15", "20"],
                showQuickJumper: true,
                showSizeChanger: true
            },
        };
        // 当前登陆人为系统管理员需展示退款申请所属机构，为机构管理员则不展示
                    ...
                    render: (text, record) => {
                        return (
                            <div className="editable-row-operations">
                                ...
                                {/*state为1的项显示审核通过按钮*/}
                                <Popconfirm title="确认通过审核?"
                                            placement="topRight"
                                            onConfirm={() => this.getUrl(record.id)}
                                            onCancel=""
                                            okType="danger"
                                            okText="确认"
                                            cancelText="取消">
                                    <a style={{display: record.state === 1 ? "inline" : "none"}}>审核通过</a>
                                </Popconfirm>
                            </div>
                        )
                    }
                    ...
                    render: (text, record) => {
                        return (
                            <div className="editable-row-operations">
                                <ItemDetails data={this.state.originalData[record.index - 1]}/>
                                {/*state为0的项显示审核按钮*/}
                                {/*signIn为true表明用户已使用课程，此时具体退款金额由机构裁定，institutionAmount为该笔交易全款金额，即可退款的最大金额*/}
                                <ItemCheck01 id={record.id} type={record.typeCode} recapture={this.getData}
                                             signIn={record.signIn}
                                             institutionAmount={record.institutionAmount}
    ...
    getData = (status) => {
        ...
        if (Number(sessionStorage.EId) === 0) {
            // 系统管理员分支
            reqwest({
                url: '/order/getRefundsExamineList',
                type: 'json',
                method: 'post',
                data: {
    ...
    handleTableChange = (pagination) => {
        ...
        localStorage.refundCheckPageSize = pagination.pageSize;
        pager.pageSize = Number(localStorage.refundCheckPageSize);
        this.setState({
            pagination: pager
        }, () => {
            this.getData()
...
class RefundCheck extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            status: Number(sessionStorage.EId) === 0 ? "1" : "0",
            // 退款申请状态选项列表
            optionsOfStatus: [],
            flag_add: false
        }
    ...
    // 根据当前登录人身份设置其可见的退款申请状态选项列表
    setOptionsOfStatus = () => {
        this.setState({
            optionsOfStatus:
                Number(sessionStorage.EId) === 0 ?
                    [
