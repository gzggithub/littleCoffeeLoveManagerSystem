...
class DataTable extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pagination: {
                current: 1,
                pageSize: Number(localStorage.institutionStatisticsPageSize) || 10,
    ...
    getData = (keyword) => {
        ...
        reqwest({
            url: '/queryPayInstitution/getStatisticsInstitutionList',
            type: 'json',
            method: 'post',
            data: {
                ...
                if (json.result === 0) {
                    ...
                    json.data.list.forEach((item, index) => {
                        data.push({
                            ...
                            type: (item.typeName || "") + ((item.typeNameTwo && item.typeNameTwo !== "0") ? ("/" + item.typeNameTwo) : "") + ((item.typeNameThree && item.typeNameThree !== "0") ? ("/" + item.typeNameThree) : ""),
    ...
    handleTableChange = (pagination, filters, sorter) => {
        ...
        localStorage.institutionStatisticsPageSize = pagination.pageSize;
        pager.pageSize = Number(localStorage.institutionStatisticsPageSize);
        this.setState({
            sortOrder: sortOrder,
            pagination: pager,
        }, () => {
...
class InstitutionStatistics extends Component {
    constructor(props) {
        super(props);
        this.state = {
            // 统计列表所需关键词
    ...
    render() {
        return (
            <div className="institutionStatistics">
