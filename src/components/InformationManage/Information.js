...
const ItemAddForm = Form.create()(
    (props) => {
        const {form, channels, viewPic, effectPic, data_pic, setViewPic, picUpload, avatarEditor, setAvatarEditor, photoLoading, provinceList, cityCode} = props;
        ...
        return (
            <Form>
                ...
                <FormItem className="typeIds" {...formItemLayout_8} label="资讯频道：">
                    {getFieldDecorator('typeIds', {
                        ...
                        <Cascader options={optionsOfChannel} placeholder="请选择资讯频道"/>
                    )}
                </FormItem>
...
const ItemEditForm = Form.create()(
    (props) => {
        const {form, data, channels, viewPic, effectPic, data_pic, setViewPic, picUpload, avatarEditor, setAvatarEditor, photoLoading, provinceList} = props;
        const {getFieldDecorator} = form;
...
class DataTable extends Component {
                    ...
                    return (
                        <div className="editable-row-operations">
                            ...
                            <a onClick={() => this.props.setEdit("status", true, record.id)}
                               style={{display: this.props.opObj.modify ? "inline" : "none"}}>编辑</a>
                            {/*资讯删除*/}
    ...
    getData = (keyword) => {
        ...
        reqwest({
            ...
            method: 'post',
            data: {
                parentId: keyword ? keyword.type : this.props.keyword.type,
                newsName: keyword ? keyword.newsName : this.props.keyword.newsName,
                ...
                if (json.result === 0) {
                    ...
                    json.data.list.forEach((item, index) => {
                        data.push({
                            ...
                            author: item.author,
                            createTime: this.dateHandle(item.createTime),
            ...
            success: (json) => {
                if (json.result === 0) {
                    this.setState({
                        ...
                        const tempArr = [<Option key={0} value="全部频道">{"全部频道"}</Option>];
                    ...
                    this.state.opObj.select ?
                        <div>
                            <header className="clearfix">
                                <Select defaultValue="全部频道" style={{width: "150px", float: "left", marginRight: "20px"}}
